...
def Authorize(session, code):
	request = app.Request('https://github.com/login/oauth/access_token', app.encodeUTF8(app.urlencode({
		'client_id': Account.CLIENT,
...
class Account(object):
	...
	@property
	def user(self):
		if not os.path.isdir(config.USER_INFO): os.makedirs(config.USER_INFO)
		...
		filepath = os.path.join(config.USER_INFO, '%s.yml' % token)
