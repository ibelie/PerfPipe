...
def route(path = '', requestMethods = (), useCookies = False, useSubmit = False):
	if not isinstance(requestMethods, (list, tuple)):
		requestMethods = (requestMethods, )
	if callable(path):
		func = path
		func.____requestMethods__ = requestMethods
...
class application(object):
	...
	def __iter__(self):
		...
		if route.____requestMethods__ and self.env['REQUEST_METHOD'] not in route.____requestMethods__:
		...
		try:
			result = self.handle_route(route, params, SimpleCookie(self.env.get('HTTP_COOKIE')), None)
		except Exception as e:
			result = self.print_trace(e)
